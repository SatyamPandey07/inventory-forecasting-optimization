...
router.get('/summary', async (req, res) => {
    try {
        ...
        // Total SKUs count
        const skuCountRes = await (0, db_1.query)(`SELECT COUNT(*) FROM skus WHERE org_id = $1`, [orgId]);
        // Active recommendations count
        const recCountRes = await (0, db_1.query)(`SELECT COUNT(*) FROM inventory_recommendations WHERE org_id = $1 AND status = 'pending'`, [orgId]);
        // Stockout risk count (SKUs where current_stock <= reorder_point)
        const stockoutRiskRes = await (0, db_1.query)(`SELECT COUNT(DISTINCT s.id) 
       FROM skus s
       LEFT JOIN LATERAL (
         SELECT units_on_hand FROM inventory_levels 
         WHERE sku_id = s.id AND org_id = $1 ORDER BY time DESC LIMIT 1
       ) i ON TRUE
       WHERE s.org_id = $1 AND COALESCE(i.units_on_hand, 0) <= s.reorder_point`, [orgId]);
        // 30-Day Demand Projected Total Revenue
        const demandRes = await (0, db_1.query)(`SELECT SUM(revenue) as total_revenue, SUM(units_sold) as total_units 
       FROM demand_history 
       WHERE org_id = $1 AND time >= NOW() - INTERVAL '30 days'`, [orgId]);
        res.json({
            summary: {
